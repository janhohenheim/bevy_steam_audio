...
/// ```
#[derive(Debug, Clone)]
pub(crate) struct TrimeshBuilder {
...
impl Trimesh {
    ...
    pub(crate) fn extend(&mut self, other: Trimesh) {
...
#[derive(Debug, Error)]
pub(crate) enum TrimeshBuilderError {
...
impl TrimeshBuilder {
    ...
    /// Translates the mesh.
    pub(crate) fn translated(&mut self, position: impl Into<Position>) -> &mut Self {
        self.position = position.into();
        self
    }

    /// Rotates the mesh.
    pub(crate) fn rotated(&mut self, rotation: impl Into<Rotation>) -> &mut Self {
    ...
    /// Default is 16.
    pub(crate) fn fallback_subdivs(&mut self, subdivs: impl TryInto<NonZeroU32>) -> &mut Self {
    ...
    pub(crate) fn cylinder_subdivs(&mut self, subdivs: impl TryInto<NonZeroU32>) -> &mut Self {
    ...
    pub(crate) fn cone_subdivs(&mut self, subdivs: impl TryInto<NonZeroU32>) -> &mut Self {
    ...
    /// Whether a failure to trimesh a subshape in a compound shape should fail the entire build process.
    /// Default is true.
    pub(crate) fn fail_on_compound_error(&mut self, fail_on_compound_error: bool) -> &mut Self {
    ...
    pub(crate) fn build(&self) -> Result<Trimesh, TrimeshBuilderError> {
...
pub(crate) trait ColliderTrimeshBuilder {
    fn trimesh_builder(&self) -> TrimeshBuilder;
}

impl ColliderTrimeshBuilder for Collider {
    fn trimesh_builder(&self) -> TrimeshBuilder {
